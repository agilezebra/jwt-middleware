...
`refreshKeysInterval` | Arbitrarily refresh an issuer's keys in the background whenever they have not been fetched for any other reason within the given duration. Refresh runs once per issuer and is shared across all middlewares in the traefik process: if several middlewares configure different intervals for the same issuer, the shortest wins. Refresh for an issuer stops (after a grace period) once no middleware in the current dynamic configuration references it, and resumes if a later configuration references it again.
...
`unauthenticatedMethods` | A list of HTTP methods that should be allowed to pass without requiring authentication. Default: empty, meaning no methods are exempt. If specified, any requests with a method in this list will not require a valid token. Methods are matched case-insensitively. **Security note:** requests using one of these methods bypass authentication entirely (no token is validated and no claims are read) so any headers named in `headerMap` are neither populated nor removed and are forwarded to the backend exactly as the client supplied them (`removeMissingHeaders` has no effect here, as there are no claims to compare against). By their very nature these requests carry no verified identity, so the backend must not rely on any `headerMap` header (or any other client-supplied header) for such methods.
...
Firebase App Check is an example of a provider that does not provide an OpenID configuration document or a JWKS endpoint at the issuer base URL. Instead, it has a fixed JWKS endpoint.
Use the alternate `issuer`/`jwks` map notation to specify the JWKS endpoint directly:
